...
AVL.prototype.deleteElement = function (deletedValue) {  //borrar nodo hoja
  this.commands = [];
  this.cmd("SetText", 0, "Borrando " + deletedValue);
  this.cmd("Step");
  this.cmd("SetText", 0, " ");
  this.highlightID = this.nextIndex++;
...
AVL.prototype.treeDelete = function (tree, valueToDelete) {  //borrar arbol con hijos
  var leftchild = false;
  if (tree != null) {
    if (tree.parent != null) {
    ...
    if (valueToDelete < tree.data) {
      this.cmd(
        "SetText",
        0,
        valueToDelete + " < " + tree.data + ".  Busca lado izquierdo del subarbol"
      );
    } else if (valueToDelete > tree.data) {
      this.cmd(
        "SetText",
        0,
        valueToDelete + " > " + tree.data + ".  Busca lado derecho del subarbol"
      );
    } else {
      this.cmd(
        "SetText",
        0,
        valueToDelete + " == " + tree.data + ".  Encuentra nodo a borrar"
      );
    }
    this.cmd("Step");
    this.cmd("SetHighlight", tree.graphicID, 0);
    ...
    if (valueToDelete == tree.data) {
      if (tree.left == null && tree.right == null) {
        this.cmd("SetText", 0, "Nodo a borrar es una hoja.  Borrar.");
        ...
        this.cmd(
          "SetText",
          0,
          "Nodo a borrar no tiene hijo a la izquierda.  \nEstablecer padre del nodo derecho."
        );
        if (tree.parent != null) {
        ...
        this.cmd(
          "SetText",
          0,
          "Nodo a borrar no tiene hijo a la derecha.  \nEstablecer padre del nodo izquierdo."
        );
        if (tree.parent != null) {
          this.cmd("Disconnect", tree.parent.graphicID, tree.graphicID);
          this.cmd(
        ...
        this.cmd(
          "SetText",
          0,
          "Encuentra el nodo a la izquierda del subarbol."  // Predecesor
        );
        ...
        this.cmd(
          "SetText",
          0,
          "Copia los valores de la izquierda dentro del nodo a borrar."
        );
        ...
        this.cmd("Delete", this.highlightID);
        this.cmd("SetText", 0, "Quita el nodo de los valores copiados.");
        ...
        if (
          ...
          this.cmd("SetText", 0, "Ajusta la altura del arbol");
          this.cmd(
            "SetForegroundColor",
            tmp.heightLabelID,
            AVL.HIGHLIGHT_LABEL_COLOR
      ...
      if (tree.left != null) {
        this.cmd("SetText", 0, "Unwinding recursion.");  // Regresando a la posicion raiz
        this.cmd(
          "CreateHighlightCircle",
          this.highlightID,
          AVL.HIGHLIGHT_COLOR,
    ...
    this.cmd(
      "SetText",
      0,
      "Elemet " + valueToDelete + " no encontro valor a borrar"
    );
  }
};
